...
- 后续（改接口）：VPP（arXiv:2412.14803）把"解码视频"改为"取视频模型特征"；GR-1（arXiv:2312.13139）、UWM（arXiv:2504.02792）、DreamZero（arXiv:2602.15922）、Cosmos Policy（Cosmos-Policy）转向 joint 建模；LAPA（arXiv:2410.11758）拿它当"无动作标注预训练"的 baseline 并指出其瓶颈；Fast-WAM（arXiv:2603.16666）质疑测试时想象是否必要。
- 分类：tutorial（arXiv:2607.00836）与 survey（arXiv:2605.00080）都把 UniPi 当 imagine-then-execute 的起点。
