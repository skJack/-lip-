...
- 后续继承（更远）：Dreamer-v2、Dreamer-v3 继续这条线，DayDreamer 把它搬到真机；genie-generative-interactive-environments 把"在生成的环境里训 agent"放大到互联网视频规模；survey-world-model-robot-learning 把本文列为机器人 WM 的概念源头（其笔记第 7 节）；tutorial-wm-to-wam 未引用。
- 后续抛弃：
