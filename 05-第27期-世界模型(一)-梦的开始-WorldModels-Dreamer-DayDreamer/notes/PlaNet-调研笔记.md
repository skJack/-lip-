...
- 结果（附录 D，Fig 8）：对弱模型（DRNN 等）显著有帮助，对 RSSM 反而略降；附录 A 承认早期版本用了 overshooting + 固定全局 prior，最终 agent 不用。Dreamer v1 起再没用过这个正则；"多步 latent 一致性"的思路以另一种形式（rollout 上逐步的 consistency loss）回到了 TD-MPC 系列（见 tdmpc2-scalable-latent-mpc 笔记）。
...
- 建立在：VRNN（Chung 2015）、DSSM（Buesing 2018）给出确定性 + 随机状态的组合（VRNN 把生成的观测喂回模型，前向预测贵；Buesing 的模型相近但用于 hybrid agent 而非显式规划）；DVBF（Karl 2016）、PR-SSM（Doerr 2018）是同期 latent 序列模型；PETS（Chua 2018）是状态空间里的 CEM 规划；Amos 2018 的 observation overshooting 是 latent overshooting 的前身；E2C / RCE 是 latent 规划的早期尝试（局部线性 + LQR）；Visual Foresight（Finn & Levine 2017、Ebert 2018，slug visual-foresight-pixel-video-prediction）是被对比的像素空间规划路线；任务与 baseline 来自 DM Control（Tassa 2018）。
- 后续（Dreamer 线）：Dreamer v1（slug Dreamer-v1）沿用 RSSM，用在想象轨迹里训的 actor-critic 取代 CEM（critic 解决 horizon 之外的回报，actor 用穿过 latent dynamics 的解析梯度训练——正是本文 Discussion 里的两条 future work），并以 PlaNet 为 baseline；Dreamer v2（slug Dreamer-v2）把 s_t 改离散、加 KL balancing；DreamerV3（slug Dreamer-v3）、DayDreamer（slug DayDreamer）把同一 RSSM 推到 150+ 任务和真机。
- 后续（latent 规划线）：MuZero（slug muzero-value-equivalent-planning）是平行的"不重建、value-equivalent latent + MCTS（蒙特卡洛树搜索）"规划；TD-MPC2（slug tdmpc2-scalable-latent-mpc）回到 latent MPC，但用 MPPI（与 CEM 同类的采样式 MPC，softmax 加权而非 top-K 重拟合）+ 终端 value + policy prior 采样、latent 不重建像素（据我所知还会 warm-start 上一步的计划，未核实）；DINO-WM（slug dino-wm）、V-JEPA 2-AC（slug vjepa2）把同样的 CEM-MPC 搬到预训练视觉特征空间并用目标图像距离代替 reward；LeCun 2022（slug lecun-2022-path-towards-ami）的"JEPA 世界模型 + cost + MPC"蓝图可以看作 PlaNet 配方去掉重建后的通用化；tutorial（slug tutorial-wm-to-wam）把这一整支归为 state-space / latent state WM。
